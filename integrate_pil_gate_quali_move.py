...
class DepthCalculator:
    ...
    # Read source and project directories from currentFolder.txt
        ...
        yolo_model_path = "/home/vyana/codes/pipes.pt"
        self.model = YOLO(yolo_model_path)

        # Initialize flag for detecting once
        self.detection_published = False

        # Publisher for publishing class name
        self.detection_pub = rospy.Publisher('/custom_node/detection', String, queue_size=10)
    ...
    def get_unique_csv_filename(self):
        ...
        filename = f"depth_030424_{timestamp}.csv"
        return filename
    ...
    def depth_image_callback(self, depth_image_msg):
        try:
            ...
            self.detection_pub.publish(self.class_name)
            print("publishiung here")
            ...
            if hasattr(self, 'model') and self.model is not None:
                ...
                results = self.model.predict(source=self.source_folder, project=self.project_folder, name='test', exist_ok=True, save_crop=True, device=0)

                # Check if detection has already been published
                if not self.detection_published:
                    for i, result in enumerate(results):
                        for j, box in enumerate(result.boxes):
                            self.class_name = result.names[box.cls[0].item()]

                            # Publish the class name of the detected object to the ROS topic
                            self.detection_pub.publish(self.class_name)

                            # Set the flag to indicate that detection has been published
                            ...
                            with self.lock:
                                self.csv_writer.writerow([self.class_name, center_x, center_y, depth_at_center])
                                # Log coordinates to the console
                                rospy.loginfo(f"Object type: {self.class_name}, Center_X: {center_x}, Center_Y: {center_y}")
